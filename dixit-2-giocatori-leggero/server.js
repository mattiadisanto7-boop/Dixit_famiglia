...
const cardsManifest = JSON.parse(
  fs.readFileSync(path.join(publicDir, 'cards.json'), 'utf8')
);
...
function makeRoom(mode, creatorName, socket) {
  ...
  const room = {
    code,
    mode: mode === 'cooperative' ? 'cooperative' : 'competitive',
    status: 'waiting',
    phase: 'waiting',
    players: [
      {
...
function viewFor(room, playerIndex) {
  ...
  if (room.phase === 'voting' || room.phase === 'results' || room.status === 'finished') {
    base.table = room.current.table.map(cardData);
  }

  if (room.phase === 'results' || room.status === 'finished') {
    base.results = room.current.results;
  }
...
function emitRoom(room) {
  room.players.forEach((player, index) => {
    if (player.socketId) {
      io.to(player.socketId).emit('state', viewFor(room, index));
    }
  });
}
...
function finishRound(room) {
  ...
  const responder = storyteller === 0 ? 1 : 0;
  ...
  const beforeScores = room.players.map((p) => p.score);
  const gained = [0, 0];

  if (room.mode === 'competitive') {
    if (responderCorrect) gained[responder] += 2;
    else gained[storyteller] += 1;

    if (storytellerCorrect) gained[storyteller] += 2;
    else gained[responder] += 1;
...
io.on('connection', (socket) => {
    ...
    room.players.forEach((player) => {
      if (player.socketId && player.socketId !== socket.id) {
        io.to(player.socketId).emit('room-closed', {
          message: `${leavingName} ha abbandonato la stanza.`
        });
      }
    });
  ...
  socket.on('responder-submit', (payload = {}, ack) => {
    const room = roomForSocket(socket);
    if (!room || room.phase !== 'responder_select') return fail(ack, 'Non puoi fare questa azione adesso.');
    const index = playerIndexBySocket(room, socket.id);
    ...
    room.current.table = shuffle([
      room.current.storytellerCard,
      room.current.responderCard,
      ...room.current.decoys
    ]);
  ...
  socket.on('vote', (payload = {}, ack) => {
    ...
    touch(room);
    succeed(ack);
    if (room.current.votes.every((vote) => vote !== null)) finishRound(room);
    else emitRoom(room);
  });
...
setInterval(() => {
  ...
  for (const [code, room] of rooms.entries()) {
    if (now - room.lastActivity > maxIdle && room.players.every((p) => !p.connected)) {
      rooms.delete(code);
    }
  }
}, 10 * 60 * 1000).unref();
...
server.listen(PORT, HOST, () => {
  console.log(`Dixit 2 giocatori in ascolto su http://${HOST}:${PORT}`);
});
